...
function validateDirectCommand(
  ...
  validateVerificationCommandSafety(command, args, label, repository);
...
function parseMechanicalFix(value: unknown, repository: string, label: string): MechanicalFix {
  ...
  validateDirectCommand(command, args, label, repository);
...
function parseCheck(
  ...
  validateDirectCommand(batch.command, batch.args, `verification checks[${index}]`, candidateRoot);
  if (

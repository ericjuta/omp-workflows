...
The built-in `autoplan` workflow selects a practical in-scope solution and writes a detailed plan. Its final presentation includes the complete selected plan without a default character or sentence cap. The standalone `autodoc` workflow finds an already selected plan, records it in canonical documentation, verifies those documents, and never devises or implements. The built-in `autoimplement` workflow finds a clear existing plan from explicit input, conversation context, or referenced canonical documents. It blocks when no clear plan exists. An explicit plan bypasses autodoc only when a current-document receipt carries its matching plan digest; otherwise autodoc inspects and adopts or updates the canonical documents. Later invalidating evidence returns to `autoplan` followed by `autodoc`.
...
Autoimplement requires one absolute prepared repository. Implementation provenance, verification commands, publication, review, CI, and delivery remain bound to that repository. Publication records the validated plain Git base ref plus immutable base and pushed-head revisions. Reviewer execution rechecks that the tracked checkout is clean, `HEAD` still equals the published head, and the base ref still resolves to the published base before reviewing that pinned range. Pre-existing untracked and ignored files remain allowed and preserved. Autodoc applies the same absolute-repository and preservation boundary whenever it updates documentation.

### Command batch environment

`runCommandBatch` items may include optional `env` and `envUnset`. `env` is a string map overlaid on the process environment. `envUnset` is an array of names deleted after that overlay so the child does not inherit them. Workflow snapshots cannot store `undefined`, so deletion uses `envUnset` rather than a missing map value. Items may also carry an immutable `expectedCommit` and an `expectedRef` name/commit pair; these fail the item receipt before invocation when the tracked checkout or named ref moved. They still cannot set `stdin`, `shell`, or `allowNonZeroExit`. Commands run without a shell.
...
Autoimplement reviewer items keep executable `omp-reviewer` and pass `--base <immutable-base-revision>`. Immediately before launch, the command batch proves the tracked checkout and validated base ref still match the immutable publication pair. Reviewer children append `$HOME/.local/bin` and `$HOME/.bun/bin` to `PATH` when those directories are missing, and unset `GOOGLE_GENAI_USE_VERTEXAI` and `GOOGLE_CLOUD_LOCATION` so a Vertex precedence warning cannot break the reviewer's JSON event collector.
...
Sanity Check revision 2 creates child sessions directly through the documented Pi SDK. A private built-in runner uses `createAgentSession` with `SessionManager.inMemory`, one independent context per child, and only `read`, `grep`, `find`, and `ls`. Child sessions load no skills, prompt templates, themes, or context files. For a model whose provider is owned by an enabled user extension, the runner preflights extension registration, admits only the single provider owner and explicitly supplied behavior extensions, rejects replacements for built-in reads, workflow control, and child resource discovery, and settles every admitted extension on success or failure. They do not receive the workflow tool and do not save Pi session files. The sessions share the parent Node process, so they do not provide OS process isolation.
...
The workflow sends the final report through a final notification with `triggerTurn: false`, so the origin model does not produce another response. Evidence and model-facing inputs stay bounded, but the formatted final report is not truncated. The CLI, JSON or RPC stream, temporary prompt file, standard-output cap, and subprocess fallback are not retained. See [the Sanity Check plan](plans/2026-08-21-sanity-check-plan.md) for the selected implementation and test boundaries.
...
- After the workflow tool accepts an agent-step submission, assistant text written later in that turn remains visible. The next workflow message is still the workflow continuation. A deferred intent makes a workflow prompt, presentation, and factual fallback compete to provide one successor turn, so an abort cannot produce two continuation turns.

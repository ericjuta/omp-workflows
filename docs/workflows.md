...
The runner uses an explicit model and thinking override when supplied. Otherwise, it uses the configured process defaults and reports the actual selected model. It does not promise to inherit a model selected temporarily in the origin Pi TUI. Only bounded final assistant text and safe operational facts leave a live child session. Pi Workflows does not persist child prompts, reasoning, intermediate messages, tool arguments, tool results, or message history.

...
export type DecisionHostKind = "pi-tui" | "omp" | "headless";
...
export function decisionHostKind(input: {
  ...
  const env = input.env ?? {};
  const ompHost = input.ompRuntime === true || env.OMPCODE === "1";
  if (input.mode === "rpc") return ompHost ? "omp" : "headless";
  ...
  return ompHost ? "omp" : "pi-tui";
}

export function audienceChannels(
  config: DecisionChannelConfig | null,
  audience: string,
  host: { kind: DecisionHostKind },
): string[] {
  const configured = config?.audiences[audience]?.channels;
  if (configured !== undefined) return [...configured];
  if (host.kind === "pi-tui") return ["pi"];
  if (host.kind === "omp") return ["omp"];
  return [];
}

export class InteractiveDecisionSession {
  ...
  async begin(request: HumanDecisionChannelRequest): Promise<{
    attemptId: string;
    createdAt: string;
    ...
    await this.options.store.recordDelivery(
      ...
      deliveryRecord(request, this.options.channelId, attemptId, "intent", "intent", createdAt),
    );
    return { attemptId, createdAt, controller };
  }
  ...
  async run(
    ...
    body: (ctx: {
      ...
      createdAt: string;
      controller: AbortController;
    }) => Promise<HumanDecisionDeliveryResult>,
  ): Promise<HumanDecisionDeliveryResult> {
    const ctx = await this.begin(request);
    try {
      return await body(ctx);
    } finally {
      this.finish(ctx.controller);
    }
  }
  ...
  async fail(
    ...
    return { status: "failed", channel: this.options.channelId, attemptId, errorCode };
  }
  ...
  async confirm(
    ...
    await this.options.onAnswer({
      request,
      response,
      source: { channel: this.options.channelId, actorId: this.options.actorId, eventId },
      idempotencyKey: `${this.options.channelId}:${eventId}`,
    });
    await this.options.store.recordDelivery(
      request,
      this.options.channelId,
    ...
    return { status: "confirmed", channel: this.options.channelId, attemptId };
  }
  ...
  private async settleOnce(decision: SettledHumanDecision): Promise<void> {
    const prior = await this.options.store.listSettlements(
      decision.decisionId,
      this.options.channelId,
    );
    if (prior.some((record) => record.state === "confirmed")) return;
    const createdAt = new Date().toISOString();
    await this.options.store.recordSettlement(decision.decisionId, this.options.channelId, {
...
export abstract class HostDecisionChannel implements HumanDecisionChannel {
  protected constructor(
    readonly id: string,
    protected readonly session: InteractiveDecisionSession,
  ) {}
  ...
  async settle(decision: SettledHumanDecision): Promise<void> {
    await this.session.settle(decision);
  }

  abstract deliver(request: HumanDecisionChannelRequest): Promise<HumanDecisionDeliveryResult>;
}
...
export function operatorSafeText(value: string): string {
  return value
    .replaceAll("\r\n", "\n")
    .replaceAll("\r", "\n")
    .split("")

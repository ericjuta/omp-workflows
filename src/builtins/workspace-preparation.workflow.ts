...
export async function inspectWorkspace(
  ...
  if (input.preparedWorkspace !== undefined) {
    ...
    try {
      const topLevel = await resolvedPath(
        await git(candidatePath, ["rev-parse", "--show-toplevel"]),
      );
      const repositoryTopLevel = await resolvedPath(
        await git(value.repository, ["rev-parse", "--show-toplevel"]),
      );
      const candidateCommonDir = await resolvedPath(
        path.resolve(candidatePath, await git(candidatePath, ["rev-parse", "--git-common-dir"])),
      );
      const repositoryCommonDir = await resolvedPath(
        path.resolve(
          value.repository,
      ...
      if (topLevel !== (await resolvedPath(candidatePath))) {
        return blocked("The prepared workspace path no longer names its Git worktree.", [
          `prepared=${candidatePath}`,
          `actual=${topLevel}`,
        ]);
      }
      if (repositoryTopLevel !== (await resolvedPath(value.repository))) {
        ...
        const registered = listing.split("\n\n").some((entry) => {
          ...
          return (
            (lines.includes(`worktree ${path.resolve(candidatePath)}`) ||
              lines.includes(`worktree ${topLevel}`)) &&
